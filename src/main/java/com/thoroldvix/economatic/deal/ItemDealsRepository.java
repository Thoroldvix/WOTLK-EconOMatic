...
public interface ItemDealsRepository extends JpaRepository<ItemPrice, Long> {
    ...
    @Query(value = """
                ...
                s.unique_name as uniqueServerName,
                        ...
                        cast((cast((ip.market_value - ip.min_buyout) as numeric) / ip.market_value) as decimal(20, 10)) * 100 as discountPercentage
                        from item_price ip
                        join item i on ip.item_id = i.id
                        join server s on s.id = ip.server_id
                        where s.id = ?1 and ip.quantity >= ?2 and i.quality >= ?3
